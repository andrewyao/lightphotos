...
///
/// No separate exposure-gain multiply here on purpose: an earlier version of
/// this pipeline stacked a static empirical constant (`LINEAR_EXPOSURE_GAIN`,
/// tuned by comparing against Apple ImageIO's own — more aggressive —
/// rendering) on top of this already-dynamic normalize. `raw.apply_scaling()`
/// does the exact same per-file black-level/white-level headroom rescale
/// RapidRaw computes manually in its own `raw_processing.rs` (`(v - black) /
/// (white - black)`, straight from the file's metadata, confirmed against
/// `vendor/rawler-0.7.2/src/imgop/raw.rs`'s `correct_blacklevel_cfa`) — so the
/// extra constant was pure double-counting: a static "make it brighter" fudge
/// on top of a normalize that was already complete and already dynamic per
/// camera. Removed once a side-by-side against a real RapidRaw render showed
/// this pipeline coming out visibly brighter/flatter than the reference.
