...
- [ ] Task 1: Vision FFI spike, validated standalone before any integration — a small test harness/binary running `VNImageRequestHandler` + `VNDetectFaceLandmarksRequest` against known open-eye and closed-eye photos, printing landmarks (files: src/facequality.rs, Cargo.toml)
