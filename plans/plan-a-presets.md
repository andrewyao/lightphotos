...
- [x] Task 5: Presets block in the Develop panel — a plain section header above Touch Up matching the slider sections (a collapsing header hid the feature on first run), scrolling row list, `+` to save, per-row menu for rename and delete, its own delete-confirm modal (files: src/ui/develop_panel.rs, src/ui/modals.rs, src/ui/mod.rs, src/i18n.rs)
...
- [x] Task 8: `src/lr_preset.rs` — anchored `crs:` scanner reading both the attribute and child-element spellings, mapping table keyed by `SliderId` and clamped to each slider's own range, unsupported fields reported into `Preset::notes`. `denoise` comes from `LuminanceSmoothing` alone, because Camera Raw defaults `ColorNoiseReduction` to 25 and reading it would denoise every import that carries the Detail panel. No XML crate (files: src/lr_preset.rs)
