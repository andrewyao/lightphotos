...
2. **Demosaic** — new `DemosaicMode { Fast, Quality }`. `Fast` calls
   ...
   - `Quality` is implemented but **not wired into any call site** this
     ...
     **Superseded**: `plans/use-rapidraw-s-algorithm-completely-dynamic-puffin.md`
     wires `Quality` into the wasm32 Loupe (`JobKind::Preview`) path.
...
- Linear-light buffer kept through to GPU / shader-side edits+gamma
  ...
  **Superseded for the wasm32 Loupe path** by
  `plans/use-rapidraw-s-algorithm-completely-dynamic-puffin.md` (a second,
  RapidRaw-specific `raw_shader.wgsl`, display-only, mac untouched — not the
  "matching mac-side decision" this note anticipated, which is still open).
...
- Wiring `Quality` demosaic into any UI call site. **Superseded** — see the
  note above.
- Any change to mac's `image_decode.rs` RAW path. (Still true — the
  supersession above is wasm32-only.)
