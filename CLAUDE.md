...
**Catalog / persistence (`src/catalog.rs`)**: ratings + develop edits live in one app-managed SQLite database at `~/Library/Application Support/com.lightphotos/catalog.db`, keyed by normalized absolute path (single `images(path, rating, adjustments, rotation)` table). An in-memory `HashMap` mirrors the table for cheap reads; writes are single-row UPSERT/DELETE. Originals in photo folders are never touched. Legacy JSON catalogs (v1 `{ratings}`, v2 `{images}`) are auto-migrated to SQLite on first load and retired to `catalog.json.bak`. `src/paths.rs` centralizes the path-normalization logic shared by the catalog, navigation, and thumbnail layers so path identity is computed consistently everywhere.
