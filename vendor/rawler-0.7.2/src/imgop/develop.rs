...
use super::{
  ...
  sensor::xtrans::xtrans_fast::XtransFastDemosaic,
  xyz::Illuminant,
};
      ...
      intermediate = match &rawimage.photometric {
        RawPhotometricInterpretation::Cfa(config) => {
          if let Intermediate::Monochrome(pixels) = intermediate {
            ...
            if config.cfa.name.len() == 36 && config.cfa.is_rgb() && config.colors.plane_count() == 3 {
              let xtrans = XtransFastDemosaic::new();
              Intermediate::ThreeColor(xtrans.demosaic(&pixels, &config.cfa, &config.colors, roi))
            } else if config.cfa.is_rgb() && config.colors.plane_count() == 3 {
